...
A reverse-lookup glossary: turn a vague description of a motion effect ("the bouncy thing when a popover opens", "the iOS rubber-band scroll") into its precise term, so you know what to ask an AI or designer for. Companion reference for the [Motion audit & upgrade](prompt%20library.md) prompt — use it to name a gap precisely instead of describing it loosely.

**How to use:** describe what you *see* or *feel*, not the technical name — map the sensation to a term below. When two terms compete (*Clip-path* vs *Mask*, *Pop in* vs *Bounce*, *Shared element transition* vs *Layout animation*), the differences are noted so you can pick the right one. If nothing here matches exactly, say so rather than inventing a term — describe the effect using the closest vocabulary instead.
